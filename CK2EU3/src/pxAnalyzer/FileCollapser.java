...
public class FileCollapser {
	 ...
	 */
	static protected class ExploredField implements Comparable<Object> {
		enum FieldType { BASE('B'), ANON('A'), STRUCT('S'), LIST('L'), EMPTY('E'); char _c; FieldType(char c){_c=c;}; };
		//name of the field
		String                     _name;
		//list of subfields (a repeated field is present only once)
		LinkedList<ExploredField>  _subFields;
		//values found for the field
		SortedSet<String>          _values;
		//known types for the field (normally, at most two (with EMPTY beeing eiter List or Struct)
		Set<FieldType>             _types = new TreeSet<FieldType>();
		//position where this field has been found (duplicates are ignored for this purpose)
		//positions start at 1 (0 means the field did not exist in that thread)
		//this will be used to consolidate the names in the order they are found
		short                      _pos[] = new short[16];
		//number of thread where this field did match ; used when putting fields in order ; limited by __limit
		int                        _matches;
		//number of thread where this field did actually match ; not limited
		int                        _total;
		//sum of the current position ; used when putting fields in order
		int                        _runningPos;
		//max known number of occurrences
		int                        _occMax;
		//current number of occurrences
		int                        _occCur;
		//current position when analyzing subfields
		short                      _posCur;
		//last field seen when analyzing subfields
		String                     _lastField;
		//current thread ; limited by __limit
		int                        _thread;
		//actual current thread ; unlimited.
		int                        _totalThread;
		//owner field
		...
		public int compareTo(Object f) {
			if (f instanceof ExploredField)
			    return _name.compareTo(((ExploredField)f)._name);
			if (f instanceof String)
			    return _name.compareTo((String)f);
		...
		ExploredField findField(String name) {
			...
			for (ExploredField f : _subFields)
				if (f.compareTo(name)==0) return f;
			return null;
		}
		...
		//ordering the subfields
		//substract 1 from all other fields positions where the first field was present
		//returns true if the field being adjusted is now a candidate for being first
		...
		//update all subfields by adjusting their positions depending on the current
		//first field positions. Returns a new candidate for first. This supposes the
		//current first field is now not in the subfields.
		ExploredField updateAllPos(ExploredField orig) {
			ExploredField first=null;
			for (ExploredField f : _subFields)
				if (substractPos(orig,f) && first==null)
					first=f;
			return first;
		}
		//returns a candidate for first field.
		ExploredField getFirst() {
			for (ExploredField f : _subFields)
			    if (f._matches==f._runningPos)
			    	return f;
			return null;
		}
		boolean orderFields() {
		...
		private void print(Writer out, boolean full, int depth) throws IOException {
			...
			for (int i=0; i<tab.length; i++) tab[i]=' '; tab[0]='\n';
			if (_name!=null) {
				out.write(tab);
				if (_name.length()>0) out.write(_name);
				else                  out.write("*");
				if (!(_types.contains(FieldType.BASE) && _types.size()==1)) {
				    char x[] = new char[_types.size()];
				    int i=0;
				    for (FieldType c : _types) x[i++]=c._c;
				    out.write(" (");out.write(x);out.write(") ");
				}
				if (full) {
					boolean optional = _total!=_owner._totalThread;
					if (_occMax!=1 || optional) {
					    if (_occMax>1 && optional)
					        out.write("[optional, up to "+_occMax+"]");
					    else if (optional)
					        out.write("[optional]");
					    else if (_occMax>1)
					        out.write("[up to "+_occMax+"]");
					}
					if (_values!=null) {
						out.write(" ==> ");
					    for (String s : _values) { out.write(s+", "); }
					}
				}
			}
			if (_subFields!=null) {
