...
//				f.delete();
	...
	static private void convertWars() {
		...
		for (Field<?> fx : global._data) {
			...
			if (null != battlesX) {
				...
				for (int i = 0; i < battlesX.length; i++) {
					...
					String batLocation = "1"; // hardcoded for now...
		            ...
		            boolean batLoss = !(aggAtt && attWon); // true whenever the attacker lost.
					...
					// assuming losses to be 50% on both sides, with 1000 cav / 1000 inf on both sides 
					// as well.  I could get exact loss numbers, but with nothing to compare against
					// there's no point.
					int batAttInf = 1000;
					int batAttCav = 1000;
					int batAttArt = 0;
					...
					battles[i] = new Event(year, month, day, 
							batName, 
							batLocation, 
							batAttInf,
							batAttCav, 
