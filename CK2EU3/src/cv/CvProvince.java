...
/*		boolean shipyard = false;
		...
		*/	
		...
		for (int i = 1; i <= 3000; i++) { // 3000 leaves a lot of room for expansion
			if (367 == i) {
				continue; // special handling of the Azores
			}
...
//		if (_province._id.equals("361)")) { System.out.println("Cairo"); }
	...
	static private void doAzores() throws IOException {
	    ...
	    p.takeEU3Defaults();
		if ((!OptionSection.isIN()) && (por!=null) && (
				search("227",por._provinces)!=null ||
				search("368",por._provinces)!=null ||
				search("229",por._provinces)!=null ||
				search("230",por._provinces)!=null
			)) {
			__owners.add("POR");			
		} else {
