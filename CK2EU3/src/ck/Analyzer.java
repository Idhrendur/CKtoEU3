...
import rules.CharacterCvRules.MonarchRule;

public class Analyzer {

	static public StructField  __root;
			
	String __filter = 
		"{"+
		"header,"+
		"globaldata{war{attacker,defender}},"+
		"relations{alliance{primary,secondary}},"+
		"character{id,tag,name,date,religion,culture,deathdate,attributes,traits,score,court},"+
		"province{id,religion,culture,privileges,improvements,effects,advances},"+
		"country{tag,form_of_goverment,capital,tier,ruler,laws,controlledprovinces,law_date_3,stability,badboy},"+
		"title{tag,tier,liege,holder}"+
		"}";
	...
	private Analyzer(String CKPath, String filename) throws IOException {
		...
		// and now, fixup the population of all provinces.
		for (Field<?> f : __root._data ) {
	        ...
	        if (x.get("deathdate")==null) {
	        	...
	        	Country country = ck.Country.__list.search(tag);
	        	if (null != country) {
	...
	static public String getSaveFile() {
		return "CK_Converted.eu3";
	}
	
	static public void loadAnalyzer(String CKPath, String filename) throws IOException {
