				...
				if (null != disc) {
					...
					if (3 == disc.length()) {
						...
						if (null != c._tech_group) {
							strip |= latin && "latin".equals(c._tech_group);
							...
							strip |= african && "african".equals(c._tech_group);
...
/*				if (_idNum == 534) {
		...
		// all the CK provinces will be some combination of latin, eastern, and muslim discovered.
		...
		if (african) {
			// none of these actually hit, because the african nations all have different specific province lists.
		}
		if (new_world) {
		...
		if (latin) {
			discList.addAll(CvCountry.__latin);
		}
		if (eastern) {
		...
		if (african) {
			discList.addAll(CvCountry.__african);
		}
		if (new_world) {
