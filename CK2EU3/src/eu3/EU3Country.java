...
public class EU3Country extends EU3FileChanger implements Comparable<EU3Country> {
	...
	public void applyChanges() {
		...
		applyBase("technology_group",_tech_group);  //eastern latin
		if (OptionSection.isIN()) {
			if (_tech_group.equals("latin") || _tech_group.equals("eastern") || 
				...
				_production_tech = "4";
				_government_tech = "4";					
			} else if (_tech_group.equals("african")) { 
				_land_tech = "1";				
				_naval_tech = "1";
				_trade_tech = "1";
				_production_tech = "1";
					...
					if (((year < OptionSection.getStartYear())) || 										
						...
						if (f instanceof StructField) {
							...
							if (null != sf2) {
								...
								_monarch = new Monarch(year, month, day, mName, dip, mil, adm, regent);
							}
...
/*		if (_tag.equals("TIM")) {
		...
		*/
		...
		if      (_tech_group.equals("latin")) { techGroupModifier = 1.0; }
		...
		else if (_tech_group.equals("african")) { techGroupModifier = 0.2; }
		...
		if (OptionSection.isIN()) {
			...
			double govCost = (1455 - 1450) * finalNonYearCost;
			double tradeCost = (1460 - 1453) * finalNonYearCost;
			double prodLandNavalCost = (1465 - 1450) * finalNonYearCost;	
			
			_land_tech_part = prodLandNavalCost * 0.25;
			_naval_tech_part = prodLandNavalCost * 0.25;
			_production_tech_part = prodLandNavalCost * 0.25;
			_trade_tech_part = tradeCost * 0.05;
			_government_tech_part = govCost * 0.65;
		}
		
		// figure out the colonists.  what would be the yearly gain?
		double colonists = 0.0;
		colonists += (_sliders[Slider.INNOVATIVENESS._id] * 0.4) +
					 (_sliders[Slider.MERCANTILISM._id] * 0.2) + 
					 Math.max(_sliders[Slider.LAND._id] * 0.2, 0);
		if (OptionSection.isIN()) {
			colonists = 0.0; // removed the slider bonuses and cut the amounts down.
		}
		if (null == _religion) {			 
		} else if (_religion.equals("catholic")) {
			colonists += 2.0; 
			if (OptionSection.isIN()) { colonists = 1.0; }
		} else if (_religion.equals("protestant")) { 
			colonists += 1.0; 
			if (OptionSection.isIN()) { colonists = 0.5; }
		...
		if (_tech_group.equals("latin")) { 
		    ...
		    // I don't know if this will mean that players in african tech groups 
...
//		EU3Header.writeHeader("EU3DistributionHeader.txt", pw); 
	    ...
	    if (null != _navyHome) {
	    	...
	    	boolean bigShips = _tech_group.equals("latin") ||
	...
	static public class Monarch {
		...
		public Monarch(short year, byte month, byte day, String name, byte dip, byte mil, byte adm, boolean regent) {
			if (dip>9) dip=9;
			if (mil>9) mil=9;
			if (adm>9) adm=9;
			if (dip<1) dip=1;
