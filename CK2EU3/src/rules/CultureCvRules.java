...
import pxAnalyzer.Analyzer;
import pxAnalyzer.FileLoader;
import pxAnalyzer.Token;
import pxAnalyzer.Analyzer.NoHooks;
...
public class CultureCvRules extends KeyString implements FieldLoadable {
	...
	static public class EU3Culture extends KeyString {
		public String _culture;
		public String _group;
		public EU3Culture() {}
		public EU3Culture(String c, String g) { _culture=c; _group=g; _key=c; }
		static private SearchSet<String,EU3Culture> __list = new SearchSet<String,EU3Culture>(EU3Culture.class);
		static private class BasicHooks extends NoHooks {
			int _level;
			String _culture;
			String _group;
...
//					System.out.println("Added:  " + _culture + " to:  " + _group);
					_culture = null;
				}
		...
		static public void load(String fname) throws IOException {
			Analyzer.analyze(FileLoader.load(fname,FileLoader.Mode.READ),new BasicHooks());
		}
		static public EU3Culture search(String culture) { return __list.search(culture);  }
	}
	
	static public FieldSet<CultureCvRules> __list = new FieldSet<CultureCvRules>(CultureCvRules.class);
	
	// if null : keep original culture from EU3 province
	// if capitalGroup : take capital culture
	// otherwise, convert
	static public EU3Culture convertCulture(String ckBaseCulture, String ckCulture, EU3Culture capital) {
		// if (ckBaseCulture.equals(ckCulture)) return null;
	...
	static public EU3Culture convertCulture(String ckCulture) {
	...
	static public void load(String EU3path, String Altpath, StructField root) throws IOException {
		File f = new File(Altpath+"common/cultures.txt");
		if (f.exists())
		    EU3Culture.load(Altpath+"common/cultures.txt");
		else
		    EU3Culture.load(EU3path+"common/cultures.txt");
		loadCultures(root);
	}
	
	static private void loadCultures(StructField root) {
	...
	public boolean load(Field<?> f) {
		...
		if (_EU3culture==null)
			Check.checkFatal(null, ((BaseField)h).get()+ " culture ", "EU3");
		return true;
	}
	...
	public static class WCulture implements Comparable<WCulture> {
		EU3Culture _c;
		float      _w;
		public WCulture(EU3Culture c, float w) { _c=c; _w=w; }
		public int compareTo(WCulture x) { return (_w<x._w) ? 1 : (_w==x._w) ? 0 : -1; }
	}
	
	static public class CultureCounter {
		...
		static private <T> int addTo(T[] l, float[] wl, float w, T v, int n) {
			for (int i=0; i<n; i++) if (l[i].equals(v)) { wl[i]+=w; return n; }
			l[n]=v; wl[n]+=w;
			return n+1;
		}
		...
		public EU3Culture getCulture() {
			...
			int   n=0;
			//find largest group
			for (int i=0; i<nbG; i++) if (t<weightG[i]) { t=weightG[i]; n=i; }
			//find largest culture within group
			t=0;
			int m=0;
			for (int i=0; i<nbC; i++)
				if (group[n].equals(culture[i]._group) &&  t<weightC[i]) {
					t=weightG[i]; m=i;
				}
			return culture[m];
		}
		
		//get cultures above a percentage score
